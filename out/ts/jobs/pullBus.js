...
class PullBus {
    ...
    async run() {
        ...
        let unitPulls = {};
        try {
            let unitMaxIds = await this.runner.call('$sync_units', []);
            let rowsAgo = [];
            let curRows = [];
            ...
            for (let row of curRows) {
                ...
                retCount += await this.pullBus(PullQueue, unit, queuePropsDefers, consts_1.constQueueSizeArr);
            ...
            await this.runner.logError(0, 'jobs pullBus loop error: ', (0, tool_2.getErrorString)(err));
            return -1;
        }
        return retCount;
    }
    async pullBus(PullQueueConstructor, unit, queuePropsArr, queueSizes) {
        let retCount = 0;
...
class PullQueue {
     ...
     * @returns
     */
    async processMessage(face, message) {
        ...
        try {
            ...
            await runner.logError(this.unit, `jobs pullBus loop to QueueInErr msgId=${msgId}`, (0, tool_2.getErrorString)(toQueueInErr));
            return false;
        }
    }
}
