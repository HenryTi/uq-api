...
exports.MapSaveSpace = exports.PMap = void 0;
const _ = require("lodash");
const il_1 = require("../../il");
const tokens_1 = require("../tokens");
const SBuilder_1 = require("../SBuilder");
const bookBase_1 = require("./bookBase");
const entity_1 = require("./entity");
class PMap extends bookBase_1.PBookBase {
    setName() {
        super.setName();
    ...
    parseField(lowerVar) {
        switch (lowerVar) {
            ...
            case 'order':
                ...
                this.entity.orderField = (0, il_1.smallIntField)(orderFieldName);
                break;
        }
    }
    parseOnAdd() {
        let onAddStatement = new il_1.ActionStatement( /*undefined, this.entity*/);
    ...
    scan(space) {
        ...
        if (onAddStatement !== undefined) {
            ...
            if (onAddStatement.pelement.scan(theSpace) === false)
                ok = false;
        }
        return ok;
    }
    scan2() {
        ...
        for (let key of this.entity.keys) {
            ...
            let td = dataType;
            let tuid = td.tuid;
            if (tuid !== undefined && (tuid.isOpen !== true
                && tuid.from === undefined
                && tuid.name !== '$user')) {
                ok = false;
    ...
    born(bornCode) {
        let sb = new SBuilder_1.SBuilder();
        // why? 为什么要去掉param前面的_
        ...
        this.buildQuery(sb);
        if (this.entity.from !== undefined)
            this.buildActionSync(sb);
    ...
    buildSelectAll(sb) {
        ...
        if (dataType.isId === false) {
            ...
            if (orderField !== undefined)
                sb.comma().ln().field(orderField, true);
            sb.r().lbrace();
            sb.add('into ret select ');
            sb.field(key0, false, 'b');
            for (let i = 1; i < keyLen; i++)
                sb.comma().ln().field(keys[i], false, 'b');
            for (let i = 0; i < len; i++)
                sb.comma().ln().field(fields[i], false, 'b');
            if (orderField !== undefined)
                sb.comma().ln().field(orderField, false, 'b');
    ...
    buildQuery(sb) {
        ...
        if (keyLen > 1 || orderField !== undefined) {
            ...
            if (orderField !== undefined)
                sb.comma().field(orderField, false, 'a');
        }
        sb.semi();
        sb.rbrace().semi();
        sb.ln();
...
exports.PMap = PMap;
class MapSaveSpace extends entity_1.ActionBaseSpace {
    //private varNo: number = 1;
    //private statementNo: number = 1;
    constructor(outer, map) {
...
exports.MapSaveSpace = MapSaveSpace;
