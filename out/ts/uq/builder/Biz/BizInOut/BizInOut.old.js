...
class FuncUniqueTo {
    ...
    build() {
        ...
        select.where(new sql_1.ExpAnd(new sql_1.ExpEQ(new sql_1.ExpField('base', b), new sql_1.ExpNum(this.ioAppID.unique.id)), new sql_1.ExpEQ(new sql_1.ExpField('ext', b), new sql_1.ExpVar(FuncUniqueTo.keyId)), new sql_1.ExpEQ(new sql_1.ExpField(this.fromName), new sql_1.ExpVar(this.param))));
...
class JsonObjectBase {
    constructor(context, transFuncName, props, ioPeers) {
        this.context = context;
        this.factory = context.factory;
    ...
    build() {
        ...
        for (let [name, bud] of this.props) {
            ...
            */
            objParams.push(new sql_1.ExpStr(toName), val);
        }
        return new sql_1.ExpFunc('JSON_OBJECT', ...objParams);
    }
    buildValFromBud(bud, peer) {
        let val;
        const { name, dataType } = bud;
        if (peer === undefined) {
            if (dataType === il_1.BudDataType.arr) {
                val = this.buildJsonArr(name, bud, undefined);
            }
            else {
                val = this.buildVal(bud);
            }
            ...
            switch (peer.peerType) {
                ...
                case il_1.PeerType.peerArr:
                    val = this.buildJsonArr(name, bud, peer);
                    break;
                case il_1.PeerType.peerId:
                    val = this.transID(peer, this.buildVal(bud));
                    break;
                case il_1.PeerType.peerOptions:
                    val = this.transOptions(peer, this.buildVal(bud));
                    break;
            }
        }
        /*
        ...
        */
        ...
        if (peer === undefined) {
            ...
            const { to, name: peerName } = peer;
            toName = to !== null && to !== void 0 ? to : peerName;
        }
        return { toName: toName, val };
    }
    buildJsonArr(name, budArr, peer) {
    ...
    buildKeyVal(peer) {
        ...
        if (keys === undefined || keys.length === 0) {
            return new sql_1.ExpVar(IOProc.otherSite);
        }
        let len = keys.length;
...
class JsonObjectArr extends JsonObjectBase {
    constructor(context, arrName, transFuncName, mainProps, props, peerArr) {
        super(context, transFuncName, props, peerArr === null || peerArr === void 0 ? void 0 : peerArr.peers);
...
//# sourceMappingURL=BizInOut.old.js.map