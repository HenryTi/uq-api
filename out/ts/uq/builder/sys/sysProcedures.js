...
class SysProcedures {
    ...
    coreFunc(name, dataType) {
        let p = this.context.createFunction(name, dataType);
        this.context.coreObjs.procedures.push(p);
        return p;
    }
    splitIdsTable(stats, ids, sep, dataType) {
    ...
    strToTable(stats, str, sepLn, sepField, fields) {
        let factory = this.context.factory;
        ...
        lstats.add(setLn);
        setLn.equ(ln, new sql.ExpFunc(factory.func_charindex, new sql.ExpStr(sepLn), new sql.ExpVar(str), new sql.ExpVar(c)));
        let ifLn = factory.createIf();
        lstats.add(ifLn);
        ifLn.cmp = new sql_1.ExpLE(new sql_1.ExpVar(ln), sql_1.ExpNum.num0);
        let setLnLen = factory.createSet();
        ifLn.then(setLnLen);
        setLnLen.equ(ln, new sql_1.ExpAdd(new sql_1.ExpVar(len), sql_1.ExpNum.num1));
        let insert = factory.createInsert();
        insert.ignore = true;
        insert.table = new sql.SqlVarTable(varTable.name);
        ...
        for (let i = field0.name === '$id' ? 1 : 0; i < fieldsLen; i++) {
            ...
            insert.cols.push({
                col: fn,
                val: new sql_1.ExpVar($fn)
            });
            let setField = factory.createSet();
            lstats.add(setField);
            setField.equ($fn, new sql.ExpFunc('SUBSTRING', new sql.ExpVar(str), new sql.ExpVar(c), new sql.ExpSub(new sql.ExpVar(pc), new sql.ExpVar(c))));
            if (i < fieldsLen - 1) {
                let setC = factory.createSet();
                lstats.add(setC);
                setC.equ(c, new sql_1.ExpAdd(new sql_1.ExpVar(p), sql_1.ExpNum.num1));
            }
            else {
                let setP = factory.createSet();
                lstats.add(setP);
                setP.equ(p, new sql_1.ExpVar(ln));
            }
        }
        lstats.add(insert);
...
exports.SysProcedures = SysProcedures;
//# sourceMappingURL=sysProcedures.js.map