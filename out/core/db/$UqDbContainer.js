...
import { sqlsVersion } from "./my/sqlsVersion";

export class $UqDbContainer extends DbContainer {
    constructor() {
        super(consts.$uq);
    }
    ...
    async logPerformance(tick: number, log: string, ms: number): Promise<void> {
        try {
            await this.db.call('performance', [tick, log, ms]);
        }
        catch (err) {
            logger.error(err);
            ...
            await this.db.call('performance', [Date.now(), msg, 0]);
        }
    }

    async uqLog(unit: number, uq: string, subject: string, content: string): Promise<void> {
        return await this.db.call('log', [unit, uq, subject, content]);
    }
    async uqLogError(unit: number, uq: string, subject: string, content: string): Promise<void> {
        return await this.db.call('log_error', [unit, uq, subject, content]);
    }

    async uqDbs(): Promise<any[]> {
        return await this.db.uqDbs();
    }
...
*/
//# sourceMappingURL=$UqDbContainer.js.map