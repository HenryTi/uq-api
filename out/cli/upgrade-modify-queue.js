            ...
            for (let db of dbs) {
                try {
                    ...
                    if (colModifyQueue.length > 0) {
                        tool_1.logger.debug(`${dbName} already upgraded`);
                        tool_1.logger.debug('\n');
                        continue;
                    }
                    tool_1.logger.debug(`== begin upgrade ${dbName}`);
                    let sqlAddCol = `ALTER TABLE ${dbName}.tv_$unit ADD modifyQueueMax BIGINT DEFAULT NULL;`;
                    yield runSql(sqlAddCol);
                    tool_1.logger.debug(`${dbName} tv_$unit add modifyQueueMax column`);
                    ...
                    if (hasUnit.length === 0) {
                        ...
                        tool_1.logger.debug(`${dbName} tv_$modify_queue add unique index $id_ix`);
                        ...
                        tool_1.logger.debug(`${dbName} tv_$modify_queue primary key (entity, id)`);
					...
					UPDATE ${dbName}.tv_$unit AS t1
                        ...
                        tool_1.logger.debug(`${dbName} set tv_$unit modifyQueueMax`);
                    }
                    else {
                        // 带unit的表
                        let sqlPrimaryKey = `alter table ${dbName}.tv_$modify_queue drop primary key, add primary key($unit, entity, id);`;
                        yield runSql(sqlPrimaryKey);
                        tool_1.logger.debug(`${dbName} tv_$modify_queue primary key ($unit, entity, id)`);
                        let sqlSetModifyQueueMax = `
                    ...
                    tool_1.logger.debug(`${dbName} done!`);
                    tool_1.logger.debug('\n');
                }
                catch (err) {
