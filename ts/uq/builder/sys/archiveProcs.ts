...
export class ArchiveProcedures extends SheetProcedures {
    ...
    private archiveIdProc(proc: sql.Procedure) {
    ...
    private archiveIdsProc(proc: sql.Procedure) {
        proc.addUnitUserParameter();
        let factory = this.context.factory;
        let schemaName = il.charField('schemaName', 50);
        let ids = '$ids';
        ...
        */
        ...
        if (wheres.length > 0) sel.where(new sql.ExpAnd(...wheres));
    }


    private archivesProc(proc: sql.Procedure) {
        proc.addUnitUserParameter();
        let factory = this.context.factory;
        let schemaName = il.charField('schemaName', 50);
        ...
        proc.statements.push(select);
        let wheres: sql.ExpCmp[] = [];
        wheres.push(new sql.ExpLT(
            new sql.ExpField('id', ta),
            new sql.ExpFunc(factory.func_ifnull, new sql.ExpVar(pageStart.name), new sql.ExpStr('9223372036854775807')),
        ));
