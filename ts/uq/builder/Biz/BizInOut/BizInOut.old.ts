...
import {
    ...
    , IDUnique, IOAppID, IOAppIO, IOAppIn, IOAppOut, IOPeer, IOPeerArr, IOPeerID, IOPeerOptions, IOPeers, JoinType
    , JsonTableColumn, PeerIDKey, PeerType, ProcParamType, bigIntField, charField, dateField
...
import {
    ExpAdd,
    ExpAnd, ExpAtVar, ExpComplex, ExpDatePart, ExpEQ, ExpExists, ExpField, ExpFunc, ExpFuncCustom, ExpFuncDb, ExpFuncInUq
...
import { JsonTrans } from "./JsonTrans";
import { JsonContext } from "./JsonVal";
...
abstract class IOProc<T extends IOAppIO> implements JsonContext {
    ...
    protected buildJsonTrans(): Select {
        ...
        let select = this.factory.createSelect();
        select.lock = LockType.none;
        // const job = new JsonObject(this.context, this.transFuncName, props, peers);
        // const jo = job.build();
    ...
    protected transID(peer: IOPeerID, val: ExpVal): ExpVal {
        ...
        if (unique === undefined) {
            ...
            return new ExpFuncDb('$site',
                `${site}.${id}.${this.transFuncName}`,
                new ExpVar(IOProc.otherSite),
                val);
        }
    }
        ...
        for (let [name, bud] of props) {
            ...
            if (peer === undefined) {
                ...
                switch (peer.peerType) {
                    ...
                    case PeerType.peerArr:
                        val = this.buildJsonArr(name, (bud as BizBudArr).props, (peer as IOPeerArr).peers);
                        break;
            ...
            objParams.push(
                ...
                bud.dataType !== BudDataType.arr ?
                    val
                    :
                    this.buildJsonArr(name, (bud as BizBudArr).props, (peer as IOPeerArr).peers)
            );
        }
    ...
    private buildJsonArr(arrName: string, props: Map<string, BizBud>, peers: { [name: string]: IOPeer }): ExpVal {
        let select = this.factory.createSelect();
        select.lock = LockType.none;
...
abstract class JsonObjectBase {
    ...
    protected readonly factory: Factory;
    protected readonly transFuncName: string;
    protected readonly props: Map<string, BizBud>;
    protected readonly ioPeers: IOPeers;
    protected readonly expJson: ExpVal;
    constructor(context: DbContext, transFuncName: string, props: Map<string, BizBud>, ioPeers: IOPeers) {
        this.context = context;
        this.factory = context.factory;
        this.transFuncName = transFuncName;
        this.props = props;
        this.ioPeers = ioPeers;
        this.expJson = new ExpVar(IOProc.vJson);
    }

    protected buildValInMain(bud: BizBud): ExpVal {
        let suffix: string;
        switch (bud.dataType) {
            default: debugger; throw new Error('unknown data type ' + bud.dataType);
            case BudDataType.ID:
    ...
    protected buildValInArr(bud: BizBud): ExpVal {
        const { name, dataType } = bud;
        let exp = new ExpField(name, IOProc.jsonTable);
        let peer: IOPeer;
        if (this.ioPeers !== undefined) peer = this.ioPeers[name];
        if (peer !== undefined && peer.type === PeerType.peerId) {
            return this.transID(peer as IOPeerID, exp);
        }
        switch (dataType) {
            default:
                return exp;
    ...
    protected buildDateVal(bud: BizBud) {
        let exp = new ExpFunc('JSON_VALUE', this.expJson, new ExpComplex(new ExpStr(`$."${bud.name}"`), undefined, undefined));
        return new ExpFunc('DATEDIFF', exp, new ExpStr('1970-01-01'));
    }
    ...
    build() {
        let objParams: ExpVal[] = [];
        for (let [name, bud] of this.props) {
            let peer: IOPeer;
            if (this.ioPeers !== undefined) peer = this.ioPeers.peers[name];
            let { toName, val } = this.buildValFromBud(bud, peer);
            /*
            let val: ExpVal;
            if (peer === undefined) {
            ...
            objParams.push(
                ...
                bud.dataType !== BudDataType.arr ?
                    val
                    :
                    this.buildJsonArr(name, bud as BizBudArr, peer as IOPeerArr)
            );
            */
            objParams.push(new ExpStr(toName), val);
        }
        return new ExpFunc('JSON_OBJECT', ...objParams);
    }
    ...
    protected buildJsonArr(name: string, budArr: BizBudArr, peer: IOPeerArr): ExpVal {
        const joa = new JsonObjectArr(this.context, name, this.transFuncName, this.props, budArr.props, peer);
        let ret = joa.build();
        return ret;
    }
    ...
    private buildKeyVal(peer: IOPeerID) {
        ...
        if (keys === undefined || keys.length === 0) {
            return new ExpVar(IOProc.otherSite);
        }
        let len = keys.length;
...
class JsonObject extends JsonObjectBase {
    protected buildVal(bud: BizBud): ExpVal {
        return this.buildValInMain(bud);
    }

    protected buildValFromKey(key: PeerIDKey): ExpVal {
        let { bud, peer } = key;
        let { val } = this.buildValFromBud(bud, peer);
        return val;
    }
}

class JsonObjectArr extends JsonObjectBase {
    protected arrName: string;
    protected readonly mainProps: Map<string, BizBud>;
    constructor(context: DbContext, arrName: string, transFuncName: string, mainProps: Map<string, BizBud>, props: Map<string, BizBud>, peerArr: IOPeerArr) {
        super(context, transFuncName, props, peerArr?.peers);
        this.arrName = arrName;
        this.mainProps = mainProps;
    }
    protected buildVal(bud: BizBud): ExpVal {
        return this.buildValInArr(bud);
    }
