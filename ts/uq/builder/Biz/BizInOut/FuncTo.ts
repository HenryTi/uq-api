...
abstract class FuncUniqueTo {
    ...
    protected abstract get fromErrName(): string;
    ...
    build() {
        ...
        if (unique.keys !== undefined) {
            ...
            select.where(new ExpAnd(
                new ExpEQ(new ExpField('base', b), new ExpNum(unique.id)),
                new ExpEQ(new ExpField('ext', b), new ExpVar(FuncUniqueTo.keyId)),
                new ExpEQ(new ExpField(this.fromName, a), new ExpVar(this.param)),
            ));
        }
        else {
            select.from(new EntityTable(EnumSysTable.atomUnique, false, a))
            select.where(new ExpAnd(
                new ExpEQ(new ExpField('i', a), new ExpVar(FuncUniqueTo.keyId)),
                new ExpEQ(new ExpField(this.fromName, a), new ExpVar(this.param)),
            ));
        }
        ...
        const appendErr = ioStatementBuilder.transErrorAppend(new ExpNum(this.ioAppID.id), this.fromErrName, this.param);
        iff.then(appendErr);
