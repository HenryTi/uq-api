...
export abstract class Db {
    ...
    */
    ...
    async sqlDropProc(procName: string, isFunc: boolean): Promise<any> {
        return await this.dbCaller.sqlDropProc(this.dbName, procName, isFunc);
    }
    async sqlProc(procName: string, procSql: string): Promise<any> {
        return await this.dbCaller.sqlProc(this.dbName, procName, procSql);
    }
    async buildProc(procName: string, procSql: string, isFunc: boolean): Promise<void> {
        await this.dbCaller.buildProc(this.dbName, procName, procSql, isFunc);
    }
    async buildRealProcFrom$ProcTable(proc: string): Promise<void> {
        await this.dbCaller.buildRealProcFrom$ProcTable(this.dbName, proc);
    }
    async call(proc: string, params: any[]): Promise<any> {
        return await this.dbCaller.call(this.dbName, proc, params);
    }
    async callEx(proc: string, params: any[]): Promise<any> {
        return await this.dbCaller.callEx(this.dbName, proc, params);
    }
    async tableFromProc(proc: string, params: any[]): Promise<any[]> {
        return await this.dbCaller.tableFromProc(this.dbName, proc, params);
    }
    async tablesFromProc(proc: string, params: any[]): Promise<any[][]> {
        return await this.dbCaller.tablesFromProc(this.dbName, proc, params);
    }
    async createDatabase(): Promise<void> {
        return await this.dbCaller.createDatabase(this.dbName);
    }
    async buildDatabase(): Promise<boolean> {
        return await this.dbCaller.buildDatabase(this.dbName);
    }
    ...
    async createResDb(resDbName: string): Promise<void> {
        await this.dbCaller.createResDb(resDbName);
    }
    async create$UqDb(): Promise<void> {
        await this.dbCaller.create$UqDb();
    }
    ...
    async createProcInDb(proc: string) {
        await this.dbCaller.createProcInDb(this.dbName, proc);
    }
}
